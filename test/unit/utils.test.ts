/**
 * Unit tests for pure utility functions extracted from renderer.ts.
 * These run in a Node.js environment with no DOM or Electron.
 */

// ─── Replicate the pure functions under test ──────────────
// (kept in sync with renderer.ts — if you change the logic there, update here)
...
function resolveFilePath(dir: string, href: string | null): string | null {
  if (!href) return href;
  if (/^(https?:|mailto:|ftp:|file:\/\/|\/)/.test(href)) return href;
  return dir ? `${dir}/${href}` : href;
}

function parseAdmonitionType(text: string | null | undefined): string | null {

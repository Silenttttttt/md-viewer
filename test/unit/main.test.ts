...
// ─── Replicate main.ts utilities under test ───────────────
// (avoids requiring main.ts which auto-starts the Electron app)
...
function readData(fp: string): FileData {
  return {
    filePath: fp,
    dir:  path.dirname(fp),
...
describe('getRecent / addRecent', () => {
  ...
  test('deduplicates — reopening moves file to front', () => {
    ...
    expect(r.filter((x: string) => x === '/a.md').length).toBe(1);
  });

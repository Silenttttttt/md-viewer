...
// ─── Functions under test (mirrored from renderer.js) ─────
...
function generateHeadingIds(root) {
...
describe('wrapTables', () => {
  ...
  test('table is direct child of wrapper', () => {
    ...
    d.innerHTML = table();
    wrapTables(d);
    const wrap = d.querySelector('.table-scroll');
...
describe('generateHeadingIds', () => {
  ...
  test('assigns id from text', () => {
    ...
    expect(d.querySelector('h1').id).toBe('hello-world');
  });

  test('does not overwrite existing id', () => {
    const d = headings('<h1 id="keep-me">Hello</h1>');
    expect(d.querySelector('h1').id).toBe('keep-me');
  });

  test('handles special chars', () => {
    const d = headings('<h2>Hello, World!</h2>');
    expect(d.querySelector('h2').id).toBe('hello-world');
  });

  test('handles numbers in heading', () => {
    const d = headings('<h3>Section 1.2</h3>');
    expect(d.querySelector('h3').id).toBe('section-12');
  });

  test('processes all heading levels', () => {
    const d = headings('<h1>H1</h1><h2>H2</h2><h3>H3</h3><h4>H4</h4>');
    expect(d.querySelector('h1').id).toBe('h1');
    expect(d.querySelector('h2').id).toBe('h2');
    expect(d.querySelector('h3').id).toBe('h3');
    expect(d.querySelector('h4').id).toBe('h4');
  });

  test('handles empty heading gracefully', () => {
    const d = headings('<h1></h1>');
    expect(d.querySelector('h1').id).toBeTruthy(); // random fallback id
  });
});
